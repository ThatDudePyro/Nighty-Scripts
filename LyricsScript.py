...
def clean_song_info(text):
    ...
    # Remove all variations of paused indicator with regex-like approach
    import re
    
    # Remove (paused) in any case with optional whitespace
    cleaned = re.sub(r'\s*\(paused\)\s*', '', text, flags=re.IGNORECASE)
    
    # Also handle other possible formats
    cleaned = re.sub(r'\s*\[paused\]\s*', '', cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r'\s*- paused\s*', '', cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r'\s*paused\s*
...
print("Lyrics script loaded successfully from GitHub", type_="SUCCESS"), '', cleaned, flags=re.IGNORECASE)
