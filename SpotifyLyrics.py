@nightyScript(
    name="Spotify Lyrics",
    author="0pyr",
    description="Automatically respond to trigger messages with fuzzy matching, blacklist filtering, and configurable delays.",
    usage=
        """
Fetches lyrics for your currently playing Spotify song using Genius API.

COMMANDS:
<p>lyrics - Fetch lyrics for current song
<p>lyrics config - Show configuration
<p>lyrics help - Show help information
<p>lyrics setkey <api_key> - Set Genius API key
<p>lyrics testkey - Test API key validity
<p>lyrics toggle - Toggle fallback mode
<p>lyrics threshold <0-100> - Set match threshold
<p>lyrics timeout <seconds> - Set request timeout
<p>lyrics retries <count> - Set max retry attempts
<p>lyrics clearcache - Clear cached results

NOTES:
- Get API key from: https://genius.com/api-clients
- Cache expires after 30 days
- Fallback mode provides search links when API key isn't set
"""
)
def SpotifyLyrics():
    import asyncio
